...
const weatherThemes = {
  Clear: {
    bg: 'from-[#f7b733] to-[#fc4a1a]',
    card: 'bg-white/20',
    emoji: '☀️',
  },
  Clouds: {
    bg: 'from-[#636fa4] to-[#e8cbc0]',
    card: 'bg-white/20',
    emoji: '☁️',
  },
  Rain: {
  ...
  Drizzle: {
    bg: 'from-[#3a7bd5] to-[#3a6073]',
    card: 'bg-white/10',
    emoji: '🌦️',
  },
  Thunderstorm: {
    bg: 'from-[#0f0c29] to-[#302b63]',
    card: 'bg-white/10',
    emoji: '⛈️',
  },
  Snow: {
    bg: 'from-[#e0eafc] to-[#cfdef3]',
    card: 'bg-white/30',
    emoji: '❄️',
  },
  Mist: {
    bg: 'from-[#606c88] to-[#3f4c6b]',
    card: 'bg-white/10',
    emoji: '🌫️',
  },
  default: {
    bg: 'from-[#4aacb1] to-[#1d4a6b]',
    card: 'bg-white/20',
    emoji: '🌡️',
  },
};
...
function WeatherCard({ data, unit, onToggleUnit }) {
  ...
  const feelsLike = unit === 'C' ? Math.round(data.main.feels_like) : Math.round((data.main.feels_like * 9) / 5 + 32);
  ...
  return (
    <div className={`rounded-3xl ${theme.card} backdrop-blur-md border border-white/30 p-8 text-white shadow-2xl`}>
      ...
      <div className="flex items-end gap-4 mt-6">
        <span className="text-8xl font-bold leading-none">{temp}°</span>
        <button
  ...
  return (
    <div className="mt-3">
      <div className="flex items-center justify-between mb-2">
        ...
        <button onClick={onClear} className="text-white/40 hover:text-white/70 text-xs transition-colors">Clear</button>
      </div>
      ...
      <div className="max-w-lg mx-auto px-4 py-12">
        ...
        {!wDetails && !isLoading && !error && (
          <div className="mt-16 text-center text-white/50">
            ...
            <p className="text-sm mt-1">Get real-time weather + 5-day forecast</p>
          </div>
        )}
